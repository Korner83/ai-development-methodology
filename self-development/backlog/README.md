...
methodology — an external audit caught it (F-09). **All ten epics now carry all five files.** The nine
...
- **Epic IDs:** `E<NN>` (zero-padded). Currently `E01` through `E10`. Folder names carry the `E` prefix (`E01-examples-folder`), per "Standard epic-folder structure" in [`03_epics.md`](../../methodology/03_epics.md).
- **Item IDs:** `BL-<####>` monotonic across all epics in this self-development backlog, and shared with `FUTURE.md` items so promotion needs no renumbering. Highest assigned: `BL-0055` (E10 `FUTURE.md`, 2026-08-20).
...
3. **WIP cap is 2 active epics** (not the methodology's typical 3). Raised from 1 on E02's close. See `EPICS.md` "WIP cap note" for reasoning. The cap rises after epics close and the maintainer observes the loop's behavior — though the cap has still never actually been contended, so the evidence for raising it further does not exist.
...
Bootstrapped on 2026-05-25; the bootstrap completed long ago. As of 2026-08-20: ten epics chartered, eight done, one parked (E04, will not resume), one active (E10). Item IDs run to `BL-0055`. The live rollup is [`EPICS.md`](EPICS.md) — this section records origin, not current state.
