...
A methodology written natively for mixed-contributor work is the bet this project takes. The structural choices it commits to are documented in [README "Why this exists"](../../README.md#why-this-exists) — patterns like named anti-patterns, same-format locks for humans + AI agents, DoD coupled to item frontmatter, tier matrix for autonomous loops, periodic deep-eval — each driven by a specific failure mode the maintainer encountered while running mixed-contributor work in practice.
